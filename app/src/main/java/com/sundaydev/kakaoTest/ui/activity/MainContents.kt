...
import com.sundaydev.kakaoTest.ui.people.PeopleDetailScreen
...
fun MainContents() {
    ...
    Scaffold(
        ...
        NavHost(
            ...
            composable(SCREEN_TV) { TvScreen(navController = navController) }
            ...
            composable(
                route = "$SCREEN_PEOPLE/{peopleId}",
                arguments = listOf(navArgument("peopleId") { type = NavType.IntType })
            ) { backStackEntry ->
                PeopleDetailScreen(peopleId = backStackEntry.arguments?.getInt("peopleId") ?: 0)
            }
        }
    }
...
fun getPeopleDetailRoute(peopleId: Int): String = "${SCREEN_PEOPLE}/$peopleId"
